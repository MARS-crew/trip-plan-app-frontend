...
import { BackArrow, BellIcon, Time2Icon } from '@/assets';
import type { RootStackParamList } from '@/navigation';

type NavigationProp = NativeStackNavigationProp<RootStackParamList>;

const ToggleSwitch: React.FC<{ value: boolean; onPress: () => void }> = ({ value, onPress }) => {
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.85}
      onPress={onPress}
      className={`h-7 w-11 rounded-full px-0.5 ${value ? 'bg-[#DF6C20]' : 'bg-[#D9D9D9]'}`}
  ...
  return (
    <SafeAreaView className="flex-1 bg-screenBackground" edges={['top']}>
      <View className="px-4 pt-2">
        ...
        <Text className="mt-9 ml-1 text-p1 font-semibold text-black">푸시 알림</Text>
