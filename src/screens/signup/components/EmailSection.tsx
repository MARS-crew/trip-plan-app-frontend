      ...
      <ContentContainer className="px-6 py-6">
        ...
        <View className="mb-2 flex-row items-end gap-2" onLayout={onEmailLayout}>
          ...
          <Pressable
            ...
            className={`h-[46px] min-w-[107px] items-center justify-center rounded-xl border border-borderGray bg-white ${
        ...
        {isCodeVerified ? (
          <Text className="mb-4 text-p text-statusSuccess">이메일 인증이 완료되었습니다</Text>
        ) : isEmailSent ? (
          <Text className="mb-4 text-p text-statusSuccess">
            인증번호가 발송되었습니다. 이메일을 확인해주세요.
          </Text>
        ) : null}

        {isEmailError ? (
          <Text className="mb-4 text-p text-statusError">올바른 이메일 형식을 입력해주세요.</Text>
        ) : null}
        ...
        {isCodeFieldVisible ? (
          <View>
            ...
            {isCodeError ? (
              <Text className="mt-2 text-p text-statusError">인증번호가 올바르지 않습니다.</Text>
            ) : null}
          </View>
        ) : null}
      </ContentContainer>
